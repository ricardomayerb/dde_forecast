...
* First have a look at how each variable behaves in the sample period given
graph twoway line rgdp qdate 
graph twoway line rgdp rpc qdate 
graph twoway line rgdp ri qdate 
graph twoway line rgdp fbcf qdate 
graph twoway line rgdp rm qdate
graph twoway line rgdp primario qdate
graph twoway line rgdp manuf qdate 
graph twoway line rgdp serv qdate 
graph twoway line rgdp pib qdate 
graph twoway line rgdp imp qdate 
graph twoway line rgdp imp_consumer qdate 
graph twoway line rgdp imp_intermediate qdate 
graph twoway line rgdp imp_capital qdate 
graph twoway line rgdp tax_cons_prod qdate 
graph twoway line rgdp cpi qdate 



pwcorr rgdp	rpc	ri fbcf rm primario	manuf serv pib imp imp_consumer	imp_intermediate imp_capital tax_cons_prod cpi

pwcorr D1.rgdp	D1.rpc	D1.ri D1.fbcf D1.rm D1.primario	D1.manuf D1.serv D1.pib D1.imp D1.imp_consumer	D1.imp_intermediate D1.imp_capital D1.tax_cons_prod D1.cpi
...
dfuller pib, lags(4)
dfuller imp, lags(4)
dfuller imp_consumer, lags(4)
dfuller imp_intermediate, lags(4) 
dfuller imp_capital, lags(4) 
dfuller tax_cons_prod, lags(4) 
dfuller cpi, lags(4)
...
dfuller D1.pib, lags(4)
dfuller D1.imp, lags(4)
dfuller D1.imp_consumer, lags(4)
dfuller D1.imp_intermediate, lags(4) 
dfuller D1.imp_capital, lags(4) 
dfuller D1.tax_cons_prod, lags(4) 
dfuller D1.cpi, lags(4)
** Except for CPI (which is the least correlated with gdp anyhow) all are significant at 10%
...
* var 2
qui var D1.rgdp D1.pib, lags(1/6)
* var 3
qui var D1.rgdp D1.pib, lags(1/4)
* var 4
qui var D1.rgdp D1.pib D1.manuf, lags(1/3)
* var 5
qui var D1.rgdp D1.pib D1.manuf, lags(1/4)
* var 6
qui var D1.rgdp D1.pib D1.manuf, lags(1/5)
* var 7
qui var D1.rgdp D1.pib D1.manuf D1.fbcf, lags(1/3)

* period1: 2013Q2
* h =1 
* VAR 54:
qui var D1.rgdp D1.pib if qdate<tq(2013Q2), lags(1/3)
...
* var 57:
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2013Q2), lags(1/3)
...
* var 60
qui var D1.rgdp D1.pib D1.manuf D1.fbcf if qdate<tq(2013Q2), lags(1/3)
...
* var 55: lag test say 5 best. Model stable, serial correlation not a big problem
qui var D1.rgdp D1.pib if qdate<tq(2013Q2), lags(1/6)
...
* var 58: lag test say 12 is best but that seems a bit much and gives an error. Model is stable at 4 lags, 5 and 6 lags, serial correlation not problem
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2013Q2), lags(1/4)
fcast compute p0069_, step(6)
fcstats D1.rgdp p0069_D_rgdp
* var 59
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2013Q2), lags(1/5)
...
*h=6
graph twoway line p0065_D_rgdp p0067_D_rgdp p0070_D_rgdp p000070_D_rgdp D1.rgdp qdate if qdate>tq(2012Q4) & qdate<tq(2015Q1)
...
* var 55: lag test say 5 best. Model stable, serial correlation not a big problem
qui var D1.rgdp D1.pib if qdate<tq(2013Q4), lags(1/6)
...
* var 57:
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2013Q4), lags(1/3)
fcast compute p00380_, step(1)
fcstats D1.rgdp p00380_D_rgdp
* var 58: lag test say 12 is best but that seems a bit much and gives an error. Model is stable at 4 lags, 5 and 6 lags, serial correlation not problem
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2013Q4), lags(1/4)
fcast compute p00390_, step(1)
fcstats D1.rgdp p00390_D_rgdp
* var 59
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2013Q4), lags(1/5)
fcast compute p00400_, step(1)
fcstats D1.rgdp p00400_D_rgdp
* var 60
qui var D1.rgdp D1.pib D1.manuf D1.fbcf if qdate<tq(2013Q4), lags(1/3)
...
qui var D1.rgdp D1.pib if qdate<tq(2014Q4), lags(1/6)
...
* var 89:
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2014Q4), lags(1/3)
...
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2014Q4), lags(1/4)
fcast compute p003900_, step(1)
fcstats D1.rgdp p003900_D_rgdp
* var 91
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2014Q4), lags(1/5)
fcast compute p004000_, step(1)
fcstats D1.rgdp p004000_D_rgdp
* var 92
qui var D1.rgdp D1.pib D1.manuf D1.fbcf if qdate<tq(2014Q4), lags(1/3)
...
graph twoway line p0065000_D_rgdp p0066000_D_rgdp p0067000_D_rgdp p0068000_D_rgdp p0069000_D_rgdp D1.rgdp qdate if qdate>tq(2014Q2) & qdate<tq(2016Q2)
...
* var 55: lag test say 5 best. Model stable, serial correlation not a big problem
qui var D1.rgdp D1.pib if qdate<tq(2015Q4), lags(1/6)
...
* var 57:
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2015Q4), lags(1/3)
fcast compute p0038000_, step(1)
fcstats D1.rgdp p0038000_D_rgdp
* var 58: lag test say 12 is best but that seems a bit much and gives an error. Model is stable at 4 lags, 5 and 6 lags, serial correlation not problem
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2015Q4), lags(1/4)
fcast compute p0039000_, step(1)
fcstats D1.rgdp p0039000_D_rgdp
* var 59
qui var D1.rgdp D1.pib D1.manuf if qdate<tq(2015Q4), lags(1/5)
fcast compute p00400000_, step(1)
fcstats D1.rgdp p00400000_D_rgdp
* var 60
qui var D1.rgdp D1.pib D1.manuf D1.fbcf if qdate<tq(2015Q4), lags(1/3)
...
*h=6
graph twoway line p00650000_D_rgdp p00660000_D_rgdp p00670000_D_rgdp p00680000_D_rgdp p00690000_D_rgdp p007000000_D_rgdp p0000700000_D_rgdp D1.rgdp qdate if qdate>tq(2015Q2) & qdate<tq(2017Q1)
...
*FORECAST FROM THE MODELS THAT SEEM TO WORK WELL:
* var 1 * 1 and two and three lags dont work well: 
qui var D1.rgdp D1.pib, lags(1/5)
fcast compute pVAR55_, step(6)
fcast graph pVAR55_D_rgdp, observed

* var 2: 
qui var D1.rgdp D1.pib, lags(1/6)
fcast compute pVAR55a_, step(6)
fcast graph pVAR55a_D_rgdp, observed

* var 3: 
qui var D1.rgdp D1.pib, lags(1/4)
fcast compute pVAR55c_, step(6)
fcast graph pVAR55c_D_rgdp, observed
* VAR 4: 
qui var D1.rgdp D1.pib D1.manuf, lags(1/5)
fcast compute pVAR4_, step(6)
fcast graph pVAR4_D_rgdp, observed
* VAR 5: 
qui var D1.rgdp D1.pib D1.manuf, lags(1/4)
fcast compute pVAR5_, step(6)
fcast graph pVAR5_D_rgdp, observed
